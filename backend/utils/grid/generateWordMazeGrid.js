...
function placeLetters (puzzle) {
    ...
    puzzleObj.grid[currentRow][currentCol] = puzzleObj.secretWord[0]
    puzzleObj.coordinates.push([currentRow, currentCol])

    for (let i = 1; i < puzzleObj.secretWord.length; i++) {
        let conflict = false
        let count = 0
        do{
        ...
        puzzleObj.grid[nextRow][nextCol] = puzzleObj.secretWord[i]
...
function generateGrid () {
    do {
        ...
        placeLetters()
    } while(!endPath(puzzleObj.secretWord[puzzleObj.secretWord.length-1], puzzleObj.grid))
    fillGrid()
